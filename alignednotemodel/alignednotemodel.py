...
def get_models(pitch, alignednotes, tonic_symbol, kernel_width=2.5):
    ...
    recording_distribution = get_model_distribution(pitch[:, 1])
    ...
    for key in note_models.keys():
        ...
        distribution = get_model_distribution(temp_pitch_vals,
        ...
        note_models[key]['stablepitch'] = {'Value': peak_freq, 'Unit': 'Hz'}
        ...
        distribution.vals = (distribution.vals * allhistval / stablepitch_val)
...
def get_model_distribution(pitch_vals, kernel_width=2.5, step_size=7.5):
    dummy_freq = 440.0
...
def plot(note_models, pitch_distribution, alignednotes, pitch):
    ...
    for note in alignednotes:
        ax1.plot(note['Interval'], [
            note['PerformedPitch']['Value'], note['PerformedPitch']['Value']],
    ...
    ax2.set_yticks([nm['stablepitch']['Value'] for nm in note_models.values()])
    ax2.set_yticklabels([key + ', ' + "%.1f" % val['stablepitch']['Value'] +
